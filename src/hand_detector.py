...
class HandDetector:
    ...
    def __init__(self, model_path: str = config.HAND_LANDMARKER_MODEL_PATH, max_num_hands: int = 2, min_detection_confidence: float = 0.5, min_tracking_confidence: float = 0.5, min_presence_confidence: float = 0.5):
    ...
    def _ensure_model_exists(self) -> None:
        ...
        logger.info("Downloading MediaPipe hand model from %s", self.MODEL_URL)
    ...
    def detect(self, frame: Any) -> list:
        ...
        try:
            ...
            if self._retry_count <= self._max_retries:
                logger.warning("Hand detection failed (attempt %d/%d): %s", self._retry_count, self._max_retries, exc)
            return []
        self._retry_count = 0
        ...
        for index, hand_landmarks in enumerate(results.hand_landmarks):
            ...
            if index < len(results.handedness) and results.handedness[index]:
                h_info = results.handedness[index][0]
                handedness = h_info.category_name
                handedness_conf = h_info.score
    ...
    def draw_landmarks(self, frame: Any, hands: list) -> Any:
        landmark_style = {
            index: self.drawing_spec_cls(color=self.GREEN, thickness=2, circle_radius=3)
            for index in range(21)
        }
        for fingertip_id in self.FINGERTIP_IDS:
            landmark_style[fingertip_id] = self.drawing_spec_cls(color=self.GREEN, thickness=2, circle_radius=5)

        connection_style = {
            (connection.start, connection.end): self.drawing_spec_cls(color=self.DARK_GREEN, thickness=2, circle_radius=2)
            for connection in self.hand_connections
        }

        for hand in hands:
            self.drawing_utils.draw_landmarks(
                frame, hand["raw_landmarks"], self.hand_connections, landmark_style, connection_style,
            )
            label = hand.get("handedness")
            if label:
                wrist = hand["landmarks"][0]
                conf = hand.get("handedness_confidence")
                text = f"{label} ({conf:.0%})" if conf else label
                cv2.putText(frame, text, (wrist["px"] + 10, max(20, wrist["py"] - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, self.GREEN, 2, cv2.LINE_AA)
        return frame
